...
void solve(int row,int col,int hour){
	if(hour>l)	return;
	...
	if(pipe[map[row][col]][0]==1){	//up
		...
		int k=0;	//up
		int opp_k=1;	//down
		if(map[row-1][col]!=0){		//현재 파이프가 갈 수 있는가
			if(pipe[map[row][col]][0]==pipe[map[row-1][col]][1]){	//진행가능여부 k와 opposite k
				if(0<=row-1&&row-1<n&&0<=col&&col<m){	//safe한 범위
					if(visit[row-1][col]==false){	//방문안했다면
						solve(row-1,col,hour+1);	//방문
					}	
				}
			}
		}
